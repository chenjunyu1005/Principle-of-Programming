...
public abstract class Parser extends LexAnalyzer
{
	...
	{   
		...
		String id = t;
		int counter=PushPopInstruciton(id);
		getToken();
		...
		if ( state == State.Assign )
		{
			...
			{
				getToken();
				return new Assignment("#"+counter, expr);
			}
			else
				errorMsg(4);
		}
	...
	
	{
		...
		{
			case Id:
				int counter=PushPopInstruciton(t);
				Id id = new Id("#"+counter);
				getToken();
				return id;
	...
	public static int PushPopInstruciton(String id)
	{
